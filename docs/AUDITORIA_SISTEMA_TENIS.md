...
|---|----------|--------------------|-----------|
| R1 | Kalshi/Polymarket como fuentes operativas de precio del MVP para un residente en España | La elegibilidad desde España y la equivalencia contractual con una moneyline son inciertas, y la liquidez en tenis partido a partido es dudosa fuera de finales/Slams. Quedan como experimento de fase posterior, no como pieza del MVP. Verificación en §30; donde no he podido confirmar términos actuales lo señalo | Media |
...
5. **Riesgo de discontinuidad de datos (operativo):** dependencia de repositorios comunitarios (Sackmann/derivados) y de fuentes sin SLA. Mitigación: snapshots inmutables propios, capa raw versionada, identificador maestro propio.
...
- **Solo la moneyline tiene a la vez odds históricas largas, margen bajo, liquidez máxima y baseline de mercado fuerte.** Es el único mercado donde puede ejecutarse el programa completo del documento (baseline no-vig histórico + backtest largo + calibración con muestra amplia). [INFERENCIA con soporte en §15]
...
- Los mercados de juegos (hándicap/total) exigen modelar la distribución completa de juegos (sensible a hold rates de ambos, formato y ritmo), tienen liquidación frágil ante retiradas y sin captura propia de odds no hay backtest honesto: fase 3.
- Tie-breaks y marcadores finos: varianza altísima, denominadores pequeños, margen alto → sin interés inicial.
...
Criterio de entrada por mercado: calibración retrospectiva del derivado (contra marcadores) dentro de tolerancias, odds propias capturadas ≥6 meses, reglas de settlement modeladas y test de correlación con posiciones moneyline activo (una exposición económica, no dos).
...
**Grupo 1 — Respaldados por evidencia:**

- **El precio de mercado (no-vig) es el predictor individual más fuerte; el cierre es más preciso que la apertura.** Cualquier criterio de selección debe medirse *contra* el mercado, no en el vacío. [EVIDENCIA]
- **Probabilidad calibrada + proper scoring rules** como base de la detección de value (no accuracy). [EVIDENCIA]
- **Edge frente al no-vig del cierre (CLV) como diagnóstico adelantado de habilidad**: batir sistemáticamente el cierre es señal de proceso con información; su ausencia sostenida es señal fuerte de ausencia de edge. No es prueba definitiva de rentabilidad neta. [EVIDENCIA sobre precisión del cierre; INFERENCIA sobre su uso como gate]
- **Elo (y variantes por superficie/margen) como resumen eficiente de la fuerza del jugador**, superior a ranking oficial como predictor. [EVIDENCIA]
...
- **Favorite-longshot bias** como regularidad empírica en tenis (favoritos ligeramente infravalorados, longshots sobrevalorados), con implicación práctica: desconfiar sistemáticamente del "value" detectado en cuotas altas. [EVIDENCIA, magnitud dependiente de operador y época]
...
### 15.2 Fuentes evaluadas

[Los detalles de licencia/cobertura/coste verificados y su estado se consolidan aquí; ver §30 para URLs.]

PENDIENTE_FUENTES

### 15.3 Decisiones de datos propuestas

- **Histórico de resultados/estadísticas:** snapshot congelado y versionado de los repositorios comunitarios (Sackmann y derivados) como base de investigación **no comercial**, con verificación cruzada manual contra webs oficiales; si el proyecto vira a comercial, presupuestar proveedor licenciado (Sportradar u equivalente). [DECISIÓN DE DISEÑO, confianza Alta]
- **Cuotas históricas:** Tennis-Data como columna vertebral del modo A (benchmark); documentar explícitamente la semántica de sus columnas y su ausencia de timestamp. [Confianza Alta]
- **Cuotas prospectivas:** scheduler propio de snapshots (T-24h/T-6h/T-1h/cierre) sobre fuentes con API autorizada dentro de presupuesto + registro manual del precio bet365/Winamax en el instante de decisión. Presupuesto: decisión del usuario (§27, D3). [Confianza Alta en la necesidad; Media en la fuente concreta]
- **Elo:** cálculo propio reproducible desde resultados (general + superficie + variante ponderada por tiempo), con Tennis Abstract solo como benchmark de sanity. [Confianza Alta]
...
Justificación del tope de complejidad: la literatura comparativa no muestra ventaja prospectiva robusta de modelos profundos sobre mercado+Elo+logística en tenis con datos públicos [EVIDENCIA en §30]; el coste marginal de mantenimiento sí es cierto.
